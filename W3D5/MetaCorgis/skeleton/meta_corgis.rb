...
#     @box_id = box_id
  ...
  def method_missing(name, *args)
    # Your code goes here...
    str_info = "get_#{name}_info"
    str_tastiness = "get_#{name}_tastiness"
    info = @snack_box.send(str_info,@box_id)
    tastiness = @snack_box.send(str_tastiness,@box_id)
    result = "#{name.upcase}: #{info}: #{tastiness} "
    tastiness > 30 ? "* #{result}" : result
